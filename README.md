...
- `MAX_REQUEST_PER_DAY` 每天最大请求次数，无视权限控制，默认1
...
| `MAX_REQUEST_PER_DAY`  | 可选                                   | 每天最大请求次数，可选，默认1                                |
...
A: 一种可能原因是经过 Nginx 反向代理，开启了 buffer，则 Nginx 会尝试从后端缓冲一定大小的数据再发送给浏览器。请尝试在反代参数后添加 `proxy_buffering off;`，然后重载 Nginx。其他 web server 配置同理。./license)
